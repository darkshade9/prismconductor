...
**Do NOT** poll for the answer file (the parent worker exits with you). **Do NOT** print `BLOCKED:` (this is not a failure — it's a pause). **Do NOT** print `Question:` (that sentinel is reserved for plan-mode and would double-fire the wrong UI path).
